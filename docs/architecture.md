...
Slice 5.5a places deterministic bounded discovery and its immutable family index in `internal/fontglyph/discovery`, and the 128-face/256-MiB singleflight parsed-face cache in `internal/fontglyph/cache`. The root `internal/fontglyph` package remains the compatibility/orchestration facade. `cache.Lease` owns one pin and closes idempotently; cached parsed values are held by `internal/fontglyph/internal/face.Owner`, so eviction closes attached native resources through the face owner. Discovery imports only `fontdesc`; cache imports only the private face owner. Resolution, shaping, raster/color and platform/native behavior remain in the facade until Slices 5.5b and 5.5c.
