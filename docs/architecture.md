...
Slice 5.5a places deterministic bounded discovery and its immutable family index in `internal/fontglyph/discovery`, and the 128-face/256-MiB singleflight parsed-face cache in `internal/fontglyph/cache`. Slice 5.5b places deterministic face planning, authored rule/fallback policy, bounded resolution/load-failure caches, portable/default shaping and run-substitution decisions in `internal/fontglyph/shape`. The root `internal/fontglyph` package remains the concrete compatibility/orchestration facade and owns cross-subsystem construction. `cache.Lease` owns one pin and closes idempotently; cached parsed values are held by `internal/fontglyph/internal/face.Owner`, while `internal/fontglyph/internal/face.Ref` exposes only immutable shaping state. Discovery imports only `fontdesc`; cache imports only the private face owner; shape imports only the private face leaf, `fontdesc`, `unicodecluster`, `unicodeprops`, and stdlib/x dependencies. Raster/color and platform/native ownership remain in the facade until Slice 5.5c.
