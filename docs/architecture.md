...
Slice 5.5a places deterministic bounded discovery and its immutable family index in `internal/fontglyph/discovery`, and the 128-face/256-MiB singleflight parsed-face cache in `internal/fontglyph/cache`. Slice 5.5b places deterministic face planning, authored rule/fallback policy, bounded resolution/load-failure caches, portable/default shaping and run-substitution decisions in `internal/fontglyph/shape`. Slice 5.5c places bitmap, SVG, COLR and portable outline paint in `internal/fontglyph/raster`, and DirectWrite analysis, shaping and native raster resources in `internal/fontglyph/platform`. The root `internal/fontglyph` package remains the concrete compatibility/orchestration facade and owns cross-subsystem construction and reverse resource unwinding. `cache.Lease` owns one pin and closes idempotently; cached parsed values are held by `internal/fontglyph/internal/face.Owner`, while `internal/fontglyph/internal/face.Ref` exposes only immutable shaping state. The complete local DAG is root to discovery/cache/shape/raster/platform/private face; discovery to `fontdesc`; cache to private face; shape/raster/platform to only private face, `fontdesc`, `unicodecluster`, and `unicodeprops`; and private face to `fontdesc`. Public subsystem packages never import root or one another.
...
### Executable mux owner capability

Slice 3.1 implements ADR-0017 through a shared `internal/ownerthread` leaf. `NewOwner` is the sole production mux constructor and binds the process owner to exact native OS-thread identity while the creating goroutine remains locked. The root GLFW `windowController` separately captures that exact native identity plus a monotonic loop epoch; every native path rechecks both before touching GLFW/OpenGL. Every process mutation and projection mutation receives its own ephemeral generation/thread/nonce scope; copies expire on leave.

Projection-local `WindowOwner` capabilities bind exact non-reusable `WindowIdentity{ID, Incarnation}`, live pane/tab topology and detached current-context attestation. Child projections retain only a typed function-message router, never the concrete root controller or process service storage. Session ingress, restore candidates and Kitty/Sixel/iTerm prepared results retain exact owner-generation stamps. `termimage` prepared transitions reacquire fresh StoreOwner scopes; close preflight holds an atomic reservation while Commit/Abort reacquire a fresh scope before topology or accounting detach.

Recursive typed current-source guards cover mux/core/termimage/frontend/ownerthread mutation closure, atomic method values, aliases, helpers, fields, function literals/captures, generics, composite containers and capability locators. Exact private-sink/body inventories, 6.2 compatibility guards, window/native-loop inventories and complete rejection fingerprints must all pass. Performance is compared only with immutable pre-slice production `9fe0bd0287ed402fe90b58a5fba7d6413897c1af` plus the hashed benchmark harness; W `27070f3` is never a baseline. L3-02 is closed only when those guards, the exact ten-sample-per-side physical ABBA gate (`<=3%` median, no allocation increase), full validation, and final history/clean-tree gates all pass together.
...
This is completed preparatory ADR-0021 parity work, not L3-01 closure. L3-01 remains partial; L3-02 closure is conditional on its complete current evidence/history gate; L3-04/L3-07/L3-09/L3-10 remain open. The facade TODO expires in Slice 6.2d, and 6.2d is deferred until its documented dependencies and execution predecessor are satisfied.
