...
**Status:** In progress. Slice 1 provides versioned documents and strict v2 validation. Slice 2 provides the candidate-only canonical source graph, declarative include traversal/guards, local module dependency capture, limits/cycles, and staged Teal collision safety. Public include merge, provenance, overrides, graph watching, and desired/effective application remain later slices.
