...
Press `ctrl+shift+r` or call `term:reload_config()` for a manual reload. Reload builds and validates a new config/runtime before changing active state; an error leaves the previous settings, watched active graph, and bindings alive and shows a notice. Colors, opacity/blur, scrollbar, and scrolling are live. Shell and other startup-only changes are reported as requiring restart. Modules first loaded later from runtime callbacks are not part of automatic dependency discovery.
