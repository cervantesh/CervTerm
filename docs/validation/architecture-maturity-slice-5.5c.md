...
All eight binary digests are independently pinned to their exact local compile records. The source manifests are historical evidence: they pin every source and fixture that existed at the Slice 5.5c base/candidate, rather than freezing the repository's future file cardinality. A successor may add Go files only while every retained non-guard manifest entry remains byte-identical. Every current production `internal/fontglyph` Go file, including root additions and additions in new packages, is recursively parsed by the ADR-0021 DAG AST scan; root-level files also enter the recursive anti-retention, ownership, and cleanup scans. Source text is LF-normalized; fixture/testdata bytes use raw exact SHA-256. The 5.5c guard itself is excluded, and dedicated maturity guard programs remain compatibility-maintained, so the guard can validate clean/pre-G/full-history successor trees without rewriting the historical manifests:
