...
The permanent `scripts/check-slice55c-evidence.go` guard validates the DAG; recursive root anti-retention; exact root field types, exported method signatures, and ABI tests; canonical ownership; semantic fuzz routing; DirectWrite constructor validation; fixture/testdata SHA-256 manifests; all eight benchmark binaries with each hash bound to its own compile row; per-run monotonic timestamp and unique nonce witnesses for the exact ABBA projections; exact ten-sample medians/allocations; exact T/A/M/W/G history; and the cleanup hash. Successor validation requires a full-history checkout so Git ancestry—not a copied-tree marker or event payload—proves that the immutable W/G lineage is present. Every shallow checkout is rejected and CI uses `fetch-depth: 0`.
...
All eight binary digests are independently pinned to their exact local compile records, and exact Slice 5.5c self-validation retains the complete historical base/candidate manifest comparison. In successor mode, the retained candidate manifest is not a repository-wide source freeze: immutable hash comparison is limited to existing `internal/fontglyph/**` source, test, fixture, license, and provenance entries plus the Slice 5.5c platform evidence runner. Unrelated frontend, core, mux, config, and other Go files, along with `go.mod` and `go.sum`, may evolve under the current global maturity size and cycle gates; safe additive packages outside `internal/fontglyph` are likewise allowed. Protected tracked `internal/fontglyph/**` paths and modes are enumerated from `git ls-files -s`: every historical pin must remain tracked with its exact content and mode, while symlink (`120000`) and gitlink (`160000`) entries are forbidden recursively. New `.go` files must be regular `100644` blobs and pass the complete ADR-0021 import-DAG scan plus root anti-retention/obsolete-authority scans where applicable. Every other new unpinned path below `internal/fontglyph` fails closed—including native/assembly/C/C++/header/`.syso` sources, fixtures, licenses, provenance, case variants, and nested testdata—until an explicit guard update adds a pin. This rejects protected rename-out, symlink relocation, alternate source extensions, and fixture-scope expansion without freezing unrelated successor work. Fixture/testdata inventory and raw/text digest rules, duplicate-fixture rejection, obsolete-root absence, exact benchmark artifacts, source pins, cleanup hash, and T/A/M/W/G ancestry remain unchanged. Source text is LF-normalized; fixture/testdata bytes use raw exact SHA-256. The 5.5c guard itself and compatibility-maintained maturity guards are excluded from successor source pins so the guard can validate clean/pre-G/full-history successor trees without rewriting the historical manifests:
