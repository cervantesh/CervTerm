...
Operational rollback is independent: set only the affected `graphics.kitty.enabled`, `graphics.sixel.enabled`, or `graphics.iterm.enabled` to `false` and restart. `--doctor` reports configured intent and limits from a separate one-shot process; it does not prove live frontend activation. The broad Phase 14 real-GUI matrix remains unrun, while later Phase 15 evidence records only narrow Windows/OpenGL iTerm PNG and WSLg/X11 Sixel visual passes. Full conformance and broad platform support remain unclaimed. See [`manual-verification.md`](manual-verification.md#phase-14-experimental-sixel-and-iterm-images--real-gui-qualification), [`validation/phase-14-qualification.md`](validation/phase-14-qualification.md), [`validation/phase-14-closeout.md`](validation/phase-14-closeout.md), and [`validation/phase-15-platform-qualification.md`](validation/phase-15-platform-qualification.md).
