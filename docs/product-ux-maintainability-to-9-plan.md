...
1. Add replayable smoke captures for `cmd.exe`, PowerShell, git/pager output, alternate screen, resize/reflow, scrollback, and Unicode/emoji layouts. **Implemented for cmd, PowerShell, git log, `more.com`, alternate screen, 40/100 column reflow, and longer delayed output in `scripts/daily-driver-smoke.ps1`.**
2. Add `scripts/daily-driver-smoke.ps1` and/or integration-tag Go tests. **Implemented.**
