...
| `ansi-smoke` | real ConPTY capture with clear-screen/private-mode noise, OSC title, and ANSI-colored `RED` text | automated in `go test ./internal/vt` |
...
go run ./scripts/build-vttest-msys2.go
go run ./scripts/capture-vttest.go -vttest dist/tools/vttest-msys2/install/bin/vttest.exe -output internal/vt/testdata/vttest-manual.vt
```
