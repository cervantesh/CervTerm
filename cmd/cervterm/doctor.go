...
func printConfigDoctor(configPath string, candidateOptions script.CandidateOptions, safeFonts bool) bool {
	...
	fmt.Println("  graphics-sixel-activation: dormant (configured intent only; frontend activation is not wired)")
	fmt.Printf("  graphics-iterm-enabled: %t\n", report.Config.Graphics.ITerm.Enabled)
	fmt.Println("  graphics-iterm-activation: dormant (configured intent only; frontend activation is not wired)")
