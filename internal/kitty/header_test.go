...
func TestParsePlacementAndDeleteNormalization(t *testing.T) {
	...
	if failure != nil || frame.command.Delete == nil || frame.command.Delete.Image == nil || !frame.command.Delete.DeleteResource {
		t.Fatalf("delete=%#v failure=%#v", frame, failure)
	}
	frame, failure = parseCompleteFrame([]byte("Ga=d,d=A"), false)
	if failure != nil || frame.command.Delete == nil || !frame.command.Delete.All || !frame.command.Delete.DeleteResource {
