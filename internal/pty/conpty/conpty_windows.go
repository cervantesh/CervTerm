...
func (c *ConPty) Spawn(argv0 string, argv []string, attr *syscall.ProcAttr) (pid int, handle uintptr, err error) {
	...
	// CREATE_NO_WINDOW: without it, Windows' default-terminal handoff treats
	// the ConPTY-hosted conhost as an unattached console app and opens a
	// visible terminal window for it, on top of CervTerm's own window.
	flags := uint32(windows.CREATE_UNICODE_ENVIRONMENT) | windows.CREATE_NO_WINDOW | extendedStartupinfoPresent
