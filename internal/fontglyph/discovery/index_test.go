...
func TestIndexLookup(t *testing.T) {
	index := &Index{families: map[string][]Face{
		"example mono": {
...
func TestBuildIndexCanonicalizesAndDeduplicatesRoots(t *testing.T) {
	root := t.TempDir()
	fontPath := filepath.Join(root, "GoMono.ttf")
	if err := os.WriteFile(fontPath, gomono.TTF, 0o600); err != nil {
		t.Fatal(err)
	}
	index := Build([]string{root, filepath.Join(root, ".")})
...
func TestBuildIndexSymlinkPolicy(t *testing.T) {
...
func TestCorruptOS2BoundsSkipOnlyFace(t *testing.T) {
	...
	index := Build([]string{filepath.Dir(path)})
	diagnostics := index.Diagnostics()
