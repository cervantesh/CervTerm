...
func Build(dirs []string) *Index {
	index := &Index{families: make(map[string][]Face)}
...
// NewIndex constructs an immutable index from detached face values.
func NewIndex(faces []Face) *Index {
	index := &Index{families: make(map[string][]Face)}
	for _, face := range faces {
		key := normalizeFamily(face.family)
		if key != "" {
			index.families[key] = append(index.families[key], face)
		}
	}
	return index
}

func (index *Index) Diagnostics() Diagnostics {
	if index == nil {
		return Diagnostics{}
	}
...
func addDiscoveryCandidate(path string, selector *topKPathSelector, diagnostics *Diagnostics) {
...
func fontFacesBounded(path string, limit int) (faces []Face, examined, truncated int, skipped bool) {
...
func SystemIndex() *Index {
	systemIndexOnce.Do(func() {
		started := time.Now()
		systemIndex = Build(SystemFontDirs())
		log.Printf("system font index scan completed in %s", time.Since(started).Round(time.Millisecond))
	})
	return systemIndex
}

func ResolveSystemFont(family string) Resolution {
	resolution := Resolution{Configured: family}
	if isEmbeddedFamily(family) {
		resolution.Found = true
		resolution.Regular = "embedded Go Mono"
		return resolution
	}
	regular, bold, italic, boldItalic := SystemIndex().Lookup(family)
	if regular == nil {
		return resolution
	}
	resolution.Found = true
