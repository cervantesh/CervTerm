...
func (a *App) refreshTabBarLayout(w, h int) tabBarLayout {
	...
	for i := range a.tabBar.Items {
		a.tabBar.Items[i].Activity = !a.tabBar.Items[i].Active && a.tabActivity[a.tabBar.Items[i].Tab]
	}
...
func (a *App) drawTabBar(w, h int) {
	...
	for _, item := range layout.Items {
		...
		if item.Activity {
			cmds = append(cmds, drawCmd{kind: cmdRect, x: float32(item.Bounds.X), y: float32(item.Bounds.Y + item.Bounds.Height - 2), w: float32(item.Bounds.Width), h: 2, col: a.chrome.accent})
		}
...
func (a *App) handleTabBarButton(button glfw.MouseButton, action glfw.Action, x, y float64) bool {
	...
	case tabHitClose:
		events, err = a.requestTabClose(hit.Tab)
	case tabHitAdd:
