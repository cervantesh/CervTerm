...
func newDescriptorFontInstallationPlanWithFactory(cfg config.Config, dpi float64, textRaster string, descriptors []fontdesc.Descriptor, construct descriptorBackendConstructor) (fontInstallationPlan, error) {
	...
	plan.fontFactory = func(spec fontglyph.Spec) (fontglyph.Backend, error) {
		key, err := makeAtlasFontKeyWithModel(spec, plan.textGamma, plan.textDarken, atlasFontModel{descriptors: factoryDescriptors, features: plan.features})
		if err != nil {
...
func newFallbackFontInstallationPlanWithFactory(cfg config.Config, dpi float64, textRaster string, descriptors, fallback []fontdesc.Descriptor, rules []fontdesc.Rule, construct fallbackBackendConstructor) (fontInstallationPlan, error) {
	...
	model := atlasFontModel{descriptors: plan.descriptors, fallback: plan.fallback, rules: plan.rules, features: plan.features}
	plan.fontFactory = func(spec fontglyph.Spec) (fontglyph.Backend, error) {
